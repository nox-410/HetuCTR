...
__global__ void generateSortkeys(index_t *dst, const index_t *d_index,
  ...
  if (id < n) {
    ...
    dst[id] = d_index[id] + r * idx_max;
  }
}

// __global__ void writePreprocessData(index_t *dst, const index_t *d_uid,
//   const worker_t *d_root, const size_t n, const size_t idx_max) {
//   size_t id = blockIdx.x * blockDim.x + threadIdx.x;
//   if (id < n) {
//     worker_t r = d_root[d_uid[id]];
//     dst[id] = r * d_index[id] + r * idx_max;
//   }
// }
...
void HetuGPUTable::preprocessIndex(unsigned long data_ptr, size_t batch_size) {
  ...
  checkCudaErrors(cub::DeviceRunLengthEncode::Encode(
    d_temp_, temp_bytes_, cur_batch_.d_unique_idx, cur_batch_.d_unique_idx, cur_batch_.d_idx_map, cur_batch_.d_idx_map,
